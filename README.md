<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:21491B,100:387D2D&height=200&section=header&text=InfoConnect&fontSize=44&fontColor=EDF4F1&animation=fadeIn&fontAlignY=36&desc=do%20movimento%20aos%20dados&descSize=18&descAlignY=58" alt="InfoConnect - do movimento aos dados" width="100%" />

**Sensores de baixo custo para medir fluxo e interesse do consumidor em supermercados.**

![Sprint](https://img.shields.io/badge/Sprint-2-387D2D?style=for-the-badge&labelColor=21491B)
![Turma](https://img.shields.io/badge/Turma-1%20ADS%20A-387D2D?style=for-the-badge&labelColor=21491B)

</div>

## 🛒 Sobre o projeto

A **InfoConnect** monitora o **fluxo e o interesse do consumidor no varejo físico**. Sensores nas gôndolas medem a distância entre o cliente e a prateleira e registram, de forma automática e anônima, **quantas pessoas passam** e **quanto tempo ficam** diante de cada produto. Os dados vão para um banco de dados e aparecem em um dashboard, ajudando supermercados e marcas a decidir melhor sobre **promoções e posicionamento de produtos**, com baixo custo e mais ROI.

```mermaid
flowchart LR
    A["📉 Falta de<br/>informações"] --> B["📡 Sensores"]
    B --> C["📈 Dados"]
    C --> D[("🗄️ Banco<br/>de dados")]
    D --> E["💻 Dashboard"]
    E --> F["🎯 Decisões<br/>assertivas"]

    classDef problem fill:#485C11,stroke:#485C11,color:#EDF4F1
    classDef step fill:#21491B,stroke:#387D2D,color:#EDF4F1
    classDef goal fill:#387D2D,stroke:#EDF4F1,color:#ffffff
    class A problem
    class B,C,D,E step
    class F goal
```


<div align="center">

![Arduino](https://img.shields.io/badge/Arduino-UNO-387D2D?style=flat-square&logo=arduino&logoColor=white&labelColor=21491B)
![MySQL](https://img.shields.io/badge/MySQL-387D2D?style=flat-square&logo=mysql&logoColor=white&labelColor=21491B)
![HTML5](https://img.shields.io/badge/HTML5-387D2D?style=flat-square&logo=html5&logoColor=white&labelColor=21491B)
![JavaScript](https://img.shields.io/badge/JavaScript-387D2D?style=flat-square&logo=javascript&logoColor=white&labelColor=21491B)
![VirtualBox](https://img.shields.io/badge/VirtualBox-387D2D?style=flat-square&logo=virtualbox&logoColor=white&labelColor=21491B)

</div>

## 🔗 Links

<!-- 🔧 Substitua os "#" pelos links reais do grupo -->

<div align="center">

| Documento | Link |
|---|---|
| Documentação | [Word](https://bandteccom-my.sharepoint.com/:w:/g/personal/fabricio_lima_sptech_school/IQC7hGp78pN3TYyNU8hrE9kaAbKvKtQYfQrLgdurBTe0jkU?e=JYOZTm) |
| Backlog | [Excel](#) |
| Tarefas | [Trello](https://trello.com/b/v9FwPukY/grupo-1-sprint-2) |
| Slides | [Apresentação](https://www.canva.com/design/DAHUdNX9vuU/XfZ0nVdsGCjIKe2flIXiyQ/edit) |
| Site Institucional | [Protótipo](https://www.figma.com/design/Xz9QYHZSdUI4lJbNYHU61Y/Prototipo-site---PI-Sprint-1?node-id=12-313&t=FWtlz2715xGLkSUN-0) |

</div>

## 👥 Equipe

<div align="center">

**Turma 1 ADS A** · SPTech School

Bianca Melo · Fabricio Lima · Alexandre Estiven · Patrick Scaglia · Thiago prado · Manoel Alves · Vitor Dias

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:387D2D,100:21491B&height=110&section=footer" alt="" width="100%" />

</div>
